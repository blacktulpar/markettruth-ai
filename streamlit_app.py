...
st.markdown(
    ...
    .mt-subtitle { color:var(--mt-muted); font-size:1.02rem; margin:.65rem 0 1.3rem 0; }
    ...
    .mt-grid { display:grid; grid-template-columns:repeat(3,minmax(0,1fr)); gap:.8rem; margin:.85rem 0 1.35rem 0; }
    .mt-kpi { background:var(--mt-panel); border:1px solid var(--mt-line); border-radius:14px; padding:1rem 1.05rem; min-height:108px; box-shadow:0 1px 2px rgba(16,24,40,.04); }
    ...
    .mt-section-title { color:var(--mt-ink); font-size:1.45rem; font-weight:800; margin:1.4rem 0 .8rem 0; }
    .mt-agent { background:var(--mt-panel); border:1px solid var(--mt-line); border-radius:16px; padding:1.05rem 1.1rem; min-height:158px; box-shadow:0 1px 2px rgba(16,24,40,.04); }
    .mt-agent-name { color:var(--mt-muted); font-size:.84rem; font-weight:700; }
    .mt-agent-bias { color:var(--mt-ink); font-size:1.75rem; font-weight:800; margin:.35rem 0 .55rem 0; }
    ...
    .mt-conflict { border:1px solid #f6b7b2; border-left:5px solid #d92d20; background:var(--mt-danger-bg); border-radius:14px; padding:1rem 1.1rem; margin:1rem 0 1.2rem 0; }
    ...
    .mt-verdict { background:#111827; color:white; border-radius:16px; padding:1.15rem 1.25rem; margin:.6rem 0 1.25rem 0; }
    ...
    .mt-evidence { background:var(--mt-panel); border:1px solid var(--mt-line); border-radius:16px; padding:1rem 1.1rem; min-height:260px; }
    .mt-evidence h4 { margin:0 0 .75rem 0; color:var(--mt-ink); font-size:1.05rem; }
    .mt-evidence ul { margin:0; padding-left:1.15rem; }
    .mt-evidence li { color:#344054; margin-bottom:.62rem; line-height:1.42; }
...
MOVE_PATH = Path("data/live/BTCUSDT.json")
CROSS_PATH = Path("data/live/NVDA_cross_market.json")
...
def hero(source_text: str) -> None:
    ...
    st.markdown(f'<div class="mt-source"><strong>Live evidence source:</strong> {esc(source_text)}</div>', unsafe_allow_html=True)
...
with st.sidebar:
    ...
    st.caption("Collect with the matching Binance Agent OS workflow first, then refresh this dashboard.")
...
try:
    ...
    if mode == "Cross Market Reality Check":
        st.info("Run prompts/cross_market_reality_check.md to create the first normalized tokenized securities snapshot.")
    st.stop()
...
if mode == "Market Move Autopsy":
    ...
    timestamp = compact_timestamp(snapshot.data_timestamp)

    hero("Binance MCP. The interface renders only the normalized investigation snapshot and never places orders.")
    st.markdown(f'<div class="mt-section-title">{esc(snapshot.symbol)} Market Move Autopsy</div>', unsafe_allow_html=True)
    st.markdown(
        f"""
        <div class="mt-grid">
            <div class="mt-kpi"><div class="mt-kpi-label">Classification</div><div class="mt-kpi-value">{esc(classification)}</div><div class="mt-kpi-sub">Truth Agent conclusion</div></div>
            <div class="mt-kpi"><div class="mt-kpi-label">Trap Risk</div><div class="mt-kpi-value">{result.trap_risk}/100</div><div class="mt-kpi-sub">Higher means more fragile conditions</div></div>
            <div class="mt-kpi"><div class="mt-kpi-label">Truth Confidence</div><div class="mt-kpi-value">{result.truth.confidence}/100</div><div class="mt-kpi-sub">Snapshot: {esc(timestamp)}</div></div>
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    with left:
        st.markdown(f'<div class="mt-agent"><div class="mt-agent-name">SPOT / MARKET STRUCTURE</div><div class="mt-agent-bias">{esc(result.spot.bias.upper())}</div><div class="mt-confidence">Confidence {result.spot.confidence}/100</div><div class="mt-bar"><span style="width:{result.spot.confidence}%"></span></div></div>', unsafe_allow_html=True)
    with right:
        st.markdown(f'<div class="mt-agent"><div class="mt-agent-name">DERIVATIVES / POSITIONING</div><div class="mt-agent-bias">{esc(result.derivatives.bias.upper())}</div><div class="mt-confidence">Confidence {result.derivatives.confidence}/100</div><div class="mt-bar"><span style="width:{result.derivatives.confidence}%"></span></div></div>', unsafe_allow_html=True)
    ...
    hero("Binance Skills Hub tokenized securities data and public Binance Web3 APIs. No trading actions are used.")
    ...
    st.markdown(
        f"""
        <div class="mt-grid">
            <div class="mt-kpi"><div class="mt-kpi-label">Classification</div><div class="mt-kpi-value">{esc(classification)}</div><div class="mt-kpi-sub">PriceTruth conclusion</div></div>
            <div class="mt-kpi"><div class="mt-kpi-label">Adjusted Gap</div><div class="mt-kpi-value">{esc(gap_label)}</div><div class="mt-kpi-sub">Token price corrected by shares multiplier</div></div>
            <div class="mt-kpi"><div class="mt-kpi-label">Misread Risk</div><div class="mt-kpi-value">{result.misread_risk}/100</div><div class="mt-kpi-sub">Risk of treating the visible gap as a simple price signal</div></div>
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    with token_col:
        st.markdown(
            f'<div class="mt-agent"><div class="mt-agent-name">TOKENIZED ASSET</div><div class="mt-agent-bias">{esc(snapshot.token_symbol or snapshot.ticker)}</div><div class="mt-confidence">Adjusted reference {esc(reference_text)}</div><div class="mt-bar"><span style="width:{result.confidence}%"></span></div></div>',
            unsafe_allow_html=True,
        )
    with context_col:
        status = snapshot.market_status or snapshot.reason_code or "Unknown"
        st.markdown(
            f'<div class="mt-agent"><div class="mt-agent-name">UNDERLYING / SESSION</div><div class="mt-agent-bias">{esc(status.upper())}</div><div class="mt-confidence">Reality confidence {result.confidence}/100</div><div class="mt-bar"><span style="width:{result.confidence}%"></span></div></div>',
            unsafe_allow_html=True,
        )
